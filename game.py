...
class Pacman(Sprite):
	...
	def load_images(self):
		for i in range(1, 13):
        # Create a new Image object (assuming you are using Pygame)
			image = pygame.image.load(f"zeldaImages/zelda{i}.png")

        # Store the loaded image in the zelda_image list
			self.zeldaImages.append(image)

        # If it's the first image, set it as the current image
			if i == 1:
				self.image = image
	...
	def draw(self, screen, cameraY):
		...
		if self.zeldaImages[imageIndex]:
			current_image = self.zeldaImages[imageIndex]

        # Blit the current image onto the screen at the sprite's position
